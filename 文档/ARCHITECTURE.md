...
`HeadlessBatchRunner` 支持来源目录、可重复 `--input` 和 TXT 密码清单。压缩来源在过滤存在性后转换为绝对规范路径并不区分大小写去重；`--source` 在压缩时展开目录直接子项，`--input` 中的目录作为单个压缩来源；解压目录只枚举匹配格式的归档。CLI 使用同步进度接收器维持 stdout/stderr 和计数顺序，`--dry-run` 不创建输出目录。

## 归档执行

`ArchiveEngineRouter` 在压缩时按规范化 `ArchiveFormat` 路由：RAR 进入 `RarArchiveEngine`，7z、ZIP、TAR、GZIP、BZIP2、XZ、WIM 进入 `SevenZipArchiveEngine`；解压时优先根据实际文件名识别格式和分卷。附件存在时作为额外根级输入，不存在时仅在暂存目录创建空目录。两个命令构建器都将每个开关与路径生成为独立参数，通用 `ArchiveProcessRunner` 仅通过 `ProcessStartInfo.ArgumentList` 启动进程，并同时异步读取标准输出和错误输出。取消会终止整个进程树。
...
格式路由必须在创建 `ArchiveOptions` 后立即进入对应引擎：RAR 只进入 `RarArchiveEngine`/`WinRarCommandBuilder`，7z 和 ZIP 只进入 `SevenZipArchiveEngine`/`SevenZipCommandBuilder`。两个构建器都使用 `ArgumentList`，但参数集合不是同一套：WinRAR 的 `-ms`、`-rr`、`-k`、`-z`、`-qo` 不得传给 7zz；7zz 的 `-t7z`、`-tzip`、`-mx`、`-mhe`、`-mem`、`-ao*` 不得传给 WinRAR。

默认文件分类和两套后端的能力限制记录在 [RAR 与 7z 后端边界](2026-08-07-RAR与7z边界.md)。
